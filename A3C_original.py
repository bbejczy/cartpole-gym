...
from torch.distributions import \
    Categorical  # Categorical import from torch.distributions module

# Hyperparameters
n_train_processes = 1  # number of independent agents. (for training)
learning_rate = 0.0002
update_interval = (
    5  # Collect data during 'update_interval steps' and proceed with training.
)
gamma = 0.98
...
class ActorCritic(
    ...
    def pi(
        ...
        prob = F.softmax(
            x, dim=softmax_dim
        )  # x[5][2] ==> Perform the softmax in the direction "dim=1". ==> batch mode: batch_size(5) x 2 # (update_interval = 5)
...
def train(global_model, rank):  # Called by 3(n_train_processes) agents independently.
    ...
    for n_epi in range(max_train_ep):  # max_train_ep = 300
        ...
        while not done:  # CartPole-v1 forced to terminates at 500 steps.
            ...
            s_batch, a_batch, td_target_batch = (
            ...
            loss = -torch.log(pi_a) * advantage.detach() + F.smooth_l1_loss(
                local_model.v(s_batch), td_target_batch.detach()
