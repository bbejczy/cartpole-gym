...
# Hyperparameters
n_train_processes = 1      # number of independent agents. (for training)
...
class ActorCritic(nn.Module):  # ActorCritic Class - Created by inheriting nn.Module (provided by Pytorch) Class. 
    ...
    def pi(self, x, softmax_dim=0):  # In the case of batch processing, softmax_dim becomes 1. (default 0)
        ...
        prob = F.softmax(x, dim=softmax_dim) # x[5][2] ==> Perform the softmax in the direction "dim=1". ==> batch mode: batch_size(5) x 2 # (update_interval = 5)
...
def train(global_model, rank): # Called by 3(n_train_processes) agents independently.  
    ...
    optimizer = optim.Adam(global_model.parameters(), lr=learning_rate) # The optimizer updates the parameters of 'global_model'.
    ...
    for n_epi in range(max_train_ep):  #max_train_ep = 300
        ...
        while not done: # CartPole-v1 forced to terminates at 500 steps.
            ...
            s_batch, a_batch, td_target_batch = torch.tensor(s_lst, dtype=torch.float), torch.tensor(a_lst), torch.tensor(td_target_lst)
            ...
            loss = -torch.log(pi_a) * advantage.detach() + F.smooth_l1_loss(local_model.v(s_batch), td_target_batch.detach()) # This is equivalent to Actor-Critic.
...
def test(global_model):
    ...
    for n_epi in range(max_test_ep):  # max_test_ep = 400
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            print( "# of episode :{:4d}, avg score : {:7.1f}, std : {:7.1f}".format(n_epi, score/print_interval, np.std(rwd_list)) )
